...
def parse_args():
    ...
    parser.add_argument('--seqmap', default="/usr/src/mot_evaluation/seqmaps/test.txt" ,help='seqmap file', type=str)
    parser.add_argument('--track',help='Tracking result directory', type=str)
    parser.add_argument(
        '--gt', help='Ground-truth annotation directory', type=str)
    parser.add_argument('--save', help='path to save results.txt', action="store_true")
    args = parser.parse_args()
    return args

def save_result(metrics, path):

    my_dict = {"IDF1": metrics[0],
               "IDP" : metrics[1],
               "IDR" : metrics[2],
               "Rcll" : metrics[3],
               "Prcn" : metrics[4],
...
if __name__ == '__main__':
    ...
    if args.save:
        ...
        for seq in sequences:
            seq = [seq]
            metrics = evaluate_tracking(seq, args.track, args.gt)
            path = f"{args.track}{seq[0]}/result.csv"
            my_dict = {"IDF1": metrics[0],
               "IDP" : metrics[1],
               "IDR" : metrics[2],
               "Rcll" : metrics[3],
